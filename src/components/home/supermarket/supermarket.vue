<template>
  <div>
    <div v-if="haveshops">
      <van-card
        v-for="item in supershopslistinfo"
        :key="item.rowguid"
    ...
    <div v-if="!haveshops">
      <van-empty description="该区域暂时没有注册的商超" />
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      supershopsParams: { type: 2, pageNum: 1, pageSize: 30 },
      supershopslistinfo: [],
      haveshops: true,
    };
  },
  created() {
    this.getsupershoplist();
  },
  methods: {
    getshopid() {
    ...
    getsupershoplist() {
      let that = this;
      getKindShop(that.supershopsParams)
        .then((res) => {
          if (res.status === 200) {
            console.log(res.data.data);
            that.supershopslistinfo = res.data.data;
            if (that.supershopslistinfo.length === 0) {
