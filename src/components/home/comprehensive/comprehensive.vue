<template>
  <div>
    <div v-if="!haveshops">
      <van-empty description="该区域暂时没有注册的商户" />
    </div>
    <div v-if="haveshops">
      <van-card
        v-for="item in comshopslistinfo"
        :key="item.rowguid"
...
export default {
  data() {
    return {
      comshopsParams: { type: 0, pageNum: 1, pageSize: 30 },
      comshopslistinfo: [],
      haveshops: true,
    };
  },
  created() {
    this.getcompreshoplist();
  },
  methods: {
    getcomshopid() {
    ...
    getcompreshoplist() {
      let that = this;
      getKindShop(that.comshopsParams)
        .then((res) => {
          if (res.status === 200) {
            console.log(res.data.data);
            that.comshopslistinfo = res.data.data;
            if (that.comshopslistinfo.length === 0) {
