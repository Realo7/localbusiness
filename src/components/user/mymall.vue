<template>
  <div>
    <!-- 我的商铺 -->
    <div v-if="!haveshops">
      <van-empty description="该用户暂时没有注册的商城" />
    </div>
    <div v-if="haveshops">
      <van-card
      ...
      >
        ...
        <template #footer>
          <van-button size="mini">进入编辑商城信息</van-button>
        </template>
        <template #price>
          <label>商城电话：{{ item.shopphone }}</label>
        </template>
        <template #bottom>
          <!-- <div>距离****米</div> -->
        </template>
...
export default {
  ...
  methods: {
    ...
    getusershops() {
      let that = this;
      let config = { header: this.token };
      getUserShop(that.usershopsParams, config)
        .then((res) => {
          if (res.status === 200) {
